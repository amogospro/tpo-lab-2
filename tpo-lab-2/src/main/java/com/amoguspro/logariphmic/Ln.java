package com.amoguspro.logariphmic;
...
public class Ln extends LimitedIterationsExpandableFunction {
    ...
    public BigDecimal calculate(final BigDecimal x, final BigDecimal precision) throws ArithmeticException {
        ...
        if (Math.abs(X - 1) <= 1) {
            do {
                prevValue = curValue;
                curValue = curValue.add(((BigDecimal.valueOf(-1).pow(i - 1)).multiply(BigDecimal.valueOf(X - 1).pow(i))).divide(BigDecimal.valueOf(i), precision.scale(), HALF_UP));
                i++;
            ...
            do {
                prevValue = curValue;
                curValue = curValue.add((BigDecimal.valueOf(-1).pow(i - 1).divide(BigDecimal.valueOf(X - 1).pow(i), precision.scale(), HALF_UP)).divide(BigDecimal.valueOf(i), precision.scale(), HALF_UP));
                i++;
            } while (new BigDecimal("0.1").pow(precision.scale()).compareTo((prevValue.subtract(curValue)).abs()) < 0 && i < maxIterations);
